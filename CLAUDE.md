...
- **Large-order gate:** staging holds orders ≥ $2,500 / 150 lb / 5 boxes (`LARGE_ORDER_*`) and pages Mac. Release = Telegram `/release <orderNumber> [WH_CODE] [now]` or `POST /api/orders/release` (`lib/large-order-releases.js`, 14-day TTL). Nothing else lets it through; the pin must still cover the quantity. Labels > $30 each still need Mac's OK on the total before releasing.
