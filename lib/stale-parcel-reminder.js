...
async function findStale({
  ...
  const skipped = { voided: 0, otherCarrier: 0, notProsol: 0, tooRecent: 0, scanned: 0, alreadyReminded: 0, unknown: 0, openReturn: 0, siblingShipped: 0 };
  ...
  // Prosol branches match on their own paperwork, not our tracking number, so a
  // reminder quoting only a PIN is not actionable on their floor. The PO number
  // we sent them on the original order is the reference they hold.
  const poByTracking = {};
  try {
    const opsDir = path.join(__dirname, '..', 'data', 'ops-state');
    for (const f of fs.readdirSync(opsDir).filter((x) => /^\d{4}-\d{2}-\d{2}\.json$/.test(x))) {
      let day; try { day = JSON.parse(fs.readFileSync(path.join(opsDir, f), 'utf8')); } catch { continue; }
      for (const [tn, v] of Object.entries(day?.phases?.pos?.byTracking || {})) {
        if (v && v.poNumber) poByTracking[tn] = v.poNumber;
      }
    }
  } catch { /* no ops-state — reminder still works, just without PO refs */ }
  ...
  for (const s of shipments) {
    ...
    // A rebuy after the email tick leaves TWO labels on one order: the branch
    // ships on the one they were told about, and the newer one never scans.
    // That is not a stranded parcel, it is our notification gap — and chasing
    // the branch for a parcel their customer already received is worse than
    // useless. If a sibling label on this order has moved, drop it.
    const sibs = shipments.filter((x) => x.orderNumber && x.orderNumber === s.orderNumber && x.trackingNumber !== s.trackingNumber);
    let siblingShipped = false;
    for (const sib of sibs) {
      const sr = await checkScan(sib.trackingNumber);
      if (sr.known && sr.scanned) { siblingShipped = true; break; }
    }
    if (siblingShipped) { skipped.siblingShipped++; continue; }
    ...
    const rec = {
      ...
      po: poByTracking[s.trackingNumber] || null,
...
function buildBranchEmail(group) {
  const lines = group.parcels
    ...
    .map((p) => `  ${p.po ? `${p.po}   ` : ''}PIN ${p.tracking}   ${p.order ? `Amazon ${p.order}` : 'no order ref'}   label ${p.shipDate}   ${p.bizDays} business days`);
  const text = [
    'Hi,',
    '',
...
<tr><th align="left" style="padding:4px 14px 4px 0">Our PO</th><th align="left" style="padding:4px 14px 4px 0">Tracking</th><th align="left" style="padding:4px 14px 4px 0">Amazon order</th><th align="left" style="padding:4px 14px 4px 0">Label date</th><th align="left" style="padding:4px 0">Waiting</th></tr>
${group.parcels.map((p) => `<tr><td style="padding:4px 14px 4px 0"><strong>${p.po || '—'}</strong></td><td style="padding:4px 14px 4px 0">${p.tracking}</td><td style="padding:4px 14px 4px 0">${p.order || '—'}</td><td style="padding:4px 14px 4px 0">${p.shipDate}</td><td style="padding:4px 0">${p.bizDays} business days</td></tr>`).join('\n')}
</table>
