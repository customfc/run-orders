...
function deriveVendor({ asin, sku, product }) {
  ...
  if (entry && typeof entry === 'object') {
    ...
    if (prosol && prosol !== 'NON_PROSOL' && !prosol.startsWith('UNMAPPED')) {
      // Has a valid Prosol SKU → route through Prosol
      // But override if product name is clearly Aqua Mix / Bona (Prosol distributes those too,
      // but the user may want them on a separate PO).
      const pname = (entry.product || product || '').toLowerCase();
      if (/aqua[\s.-]*mix/i.test(pname)) return 'aquamix';
      if (/bona/i.test(pname)) return 'bona';
      if (/perfect[\s.-]*level/i.test(pname)) return 'perfectlevel';
      return 'prosol';
    }
  }

  // 2. Fallback on product-name regex (SKUs missing from map)
