...
const VENDOR_META = {
  prosol: {
    label: 'Prosol',
    primary_brand: 'Schluter',
    email: 'klazzarotto@prosol.ca',
    contact: 'Kaitlyn Lazzarotto',
    note: 'Primary distributor for Schluter products',
  },
  bona: {
    label: 'Bona',
    email: null, // TODO: fill in
    contact: null,
...
function deriveVendor({ asin, sku, product }) {
  ...
  // 1. sku-map brand — any product with a valid Prosol SKU is ordered from Prosol.
  //    Prosol distributes Schluter, Aqua Mix, and others. Brand-based vendor
  //    override only applies when the user explicitly orders from a different
  //    distributor (currently only for Bona / Perfect Level — confirm before
  //    splitting those from Prosol).
  if (entry && typeof entry === 'object') {
    if ((entry.brand || '').toLowerCase() === 'schluter') return 'prosol';
  ...
  // 2. Fallback on product-name regex (SKUs missing from map entirely)
  const pname = (product || entry?.product || '').toLowerCase();
  if (/^bona\b|\bbonakemi\b/i.test(pname)) return 'bona';
