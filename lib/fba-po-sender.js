...
//
function renderProsolBody(lines, draftId, { bucket, fnskuAttached } = {}) {
  ...
  if (fnskuAttached) {
    ...
    html += `<strong>FNSKU labels — apply as you pull:</strong>`;
    html += `<ol style="margin:6px 0 0 22px;padding:0">`;
    html += `<li>Print the attached <strong>FNSKU PDF</strong> on the <strong>1" × 2-5/8" 30-up label sheets</strong> (Avery 5160 / generic equivalent) we keep at WCAS.</li>`;
    html += `<li>In Adobe Reader / print dialog set <strong>Page Sizing = "Actual size"</strong> — do NOT "Fit" or "Shrink to fit" or the 30-up grid won't line up.</li>`;
    html += `<li>As each bottle is pulled, apply one label <strong>directly over the existing manufacturer barcode</strong> so the FNSKU is the only scannable code.</li>`;
    html += `<li>Reply if you're running low on label sheets and we'll mail more with the next pickup.</li>`;
    html += `</ol>`;
    html += `</div>`;
  }
...
async function sendVendorGroup({ draft, vendor, bucket, skipEmail = false }) {
  ...
  const html = renderBody(vendor, lines, draft.draftId, { bucket, fnskuAttached: !!fnskuResult });
  const attachments = [];
  if (pdfBuffer) {
    attachments.push({
