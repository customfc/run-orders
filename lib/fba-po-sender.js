...
function buildSubject(vendor, lines, bucket) {
  ...
  if (vendor === 'prosol') return `Replenishment PO${bucketTag} — ${today} — ${lines.length} line${lines.length === 1 ? '' : 's'}`;
  if (vendor === 'treeco') return `Bona Replenishment PO${bucketTag} — ${today} — ${lines.length} line${lines.length === 1 ? '' : 's'}`;
