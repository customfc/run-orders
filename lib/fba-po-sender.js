...
function renderProsolBody(lines, draftId, { bucket } = {}) {
  ...
  html += `<p style="font-size:13px;color:#475569"><strong>FNSKU sheet stock:</strong> when email #2 arrives with the FNSKU PDF, please print on the <strong>1" × 2-5/8" 30-up label sheets</strong> (Avery 5160 / generic equivalent) we keep at WCAS. If you're running low, reply and we'll mail a fresh stack with the next pickup.</p>`;
...
function renderLabelsBody({ vendor, lines, bucket, fcCode, fcAddress, poNumber, shipmentConfirmationId, amazonReferenceId, cartonDims }) {
  ...
  let html = `<div style="font-family:Arial,sans-serif;font-size:14px;color:#333;line-height:1.5">`;
  html += `<p>${greeting}</p>`;
  html += `<p>Labels attached for <strong>${poNumber || 'PO-DRAFT'}</strong> (${lines.length} SKU${lines.length === 1 ? '' : 's'}, ${totalUnits} units). Amazon has confirmed the destination FC; we're good to go as soon as you're packed and labeled.</p>`;
  ...
  // Prep instructions.
  html += `<div style="margin:16px 0;padding:12px 14px;background:#fef3c7;border-left:3px solid #d97706">`;
  html += `<strong>Labeling + prep:</strong>`;
  html += `<ul style="margin:6px 0 0 22px;padding:0">`;
  html += `<li><strong>FNSKU labels</strong> (attached PDF, one per unit) — apply one to each unit so it fully covers the manufacturer barcode. Print on the <strong>1" × 2-5/8" 30-up label sheets</strong> (Avery 5160 / generic equivalent) we keep at your location. In Adobe Reader / print dialog set <strong>Page Sizing = "Actual size"</strong> — do NOT "Fit" or "Shrink to fit" or the 30-up grid won't line up. Reply if you're running low on sheets and we'll mail more with the next pickup.</li>`;
  html += `<li><strong>Transport / carton labels</strong> (attached PDF) — apply one per carton. These route the carton to the Amazon FC above via UPS Amazon Partner Carrier.</li>`;
  html += `<li>Max 50 lb / carton. No mixed SKUs per carton unless flagged in the manifest.</li>`;
  if (cartonDims) {
    html += `<li style="color:#475569"><em>Your noted dims: ${cartonDims.count} carton${cartonDims.count === 1 ? '' : 's'} @ ${cartonDims.L}×${cartonDims.W}×${cartonDims.H} in, ${cartonDims.weightLb} lb each.</em></li>`;
  }
  html += `</ul>`;
  html += `</div>`;
...
async function sendVendorGroup({ draft, vendor, bucket, skipEmail = false }) {
  const meta = VENDOR_META[vendor];
  if (!meta) throw new Error(`Unknown vendor '${vendor}'`);
...
module.exports = { preview, sendVendorGroup, sendAllBucketsForVendor, sendLabelsEmail, renderLabelsBody, archiveIfAllSent, renderBody, buildSubject, createSalesforceFbaPO, BUCKET_ORDER };
