...
async function createSalesforceFbaPO({ vendor, draft, lines }) {
  if (vendor !== 'prosol') {
    return { skipped: true, reason: `SF PO skipped — no SF vendor ID configured for '${vendor}' yet` };
  }
  ...
  // 1. Resolve each line to a PBSI item
  for (const line of lines) {
    const prosolSku = line.prosolStock?.prosolSku;
    if (!prosolSku) {
      errors.push(`No prosol_sku on line ${line.asin} (${(line.product || '').slice(0, 40)})`);
      continue;
    }
    try {
      const pbsiItem = await findPbsiItem(conn, prosolSku);
      if (!pbsiItem) {
        errors.push(`PBSI item not found for ${prosolSku} (${line.asin})`);
        continue;
      }
      resolvedLines.push({
        line,
      ...
      errors.push(`Lookup failed for ${prosolSku}: ${e.message}`);
    }
  }
