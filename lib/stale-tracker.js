...
const splitChildren = require('./split-children');
...
async function fetchTrackingDetails(labelIds, labelToShipment) {
  ...
  for (let i = 0; i < labelIds.length; i += batchSize) {
    ...
    // Throttle between batches so a large hanging backlog doesn't burst-trip the V2 rate limit.
    if (i + batchSize < labelIds.length) await new Promise((r) => setTimeout(r, 300));
  }
...
// Per-warehouse pack-time overrides. Some Prosol warehouses pack slower than
// others, so the 'wait' window before booking a pickup stretches longer.
// Key = warehouse code from prosol-location-map.json (WGRF, WCON, etc.).
// Value = max age (days) for which suggestedAction returns 'wait'.
// Default = 1 (same day + next day wait; book starting day 2).
const WAIT_AGE_BY_WAREHOUSE_CODE = {
  WGRF: 2,  // Saint-Laurent (Montreal) — packs slower, add a day
};

function suggestedAction(movement, pickupState, age, warehouseCode) {
  ...
  const waitAge = WAIT_AGE_BY_WAREHOUSE_CODE[warehouseCode] ?? 1;
  if (age <= waitAge) return 'wait';                     // warehouse still packing
  return 'book';                                         // no pickup, age past wait window
}
...
async function scanStaleShipments({ days = 14 } = {}) {
  ...
  const voidedShipmentIds = new Set();
  {
    ...
    for (let i = 0; i < ids.length; i += batchSize) {
      ...
      // Throttle between batches — same rate-limit guard as the tracking fetch above.
  ...
  // Split children first: the GET /orders/{orderId} lookup below CANNOT resolve
  // them. A multi-package child's order id is a phantom that 404s (verified on
  // 768450010 and 768450044), so every one landed in the cache as '' and stayed
  // orphaned — invisible to the stale reminder and the auto-rebooker, which is
  // why the Saint-Bruno thermostat and Richmond roll were never chased. Our own
  // buy record has the parent; use it before spending API calls on 404s.
  const repaired = splitChildren.repair(liveShipments, { knownWarehouseIds: new Set(Object.keys(WAREHOUSE_BY_SS_ID)) });
  if (repaired.orderFixed || repaired.warehouseFixed) {
    console.log(`[stale-tracker] split children restored: ${repaired.orderFixed} order number(s), ${repaired.warehouseFixed} warehouse(s)`);
  }
  ...
  for (const s of liveShipments) {
    ...
    const action = suggestedAction(movement, pickupState, age, loc?.code);
