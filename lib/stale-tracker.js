...
function classifyMovement(track) {
  if (!track) return 'hanging';
  if (track.delivered) return 'delivered';
  if (track.hasCarrierMovement) return 'in-transit';
  return 'hanging';
}
...
async function scanStaleShipments({ days = 14 } = {}) {
  ...
  for (const s of liveShipments) {
    ...
    const movement = classifyMovement(track);
  ...
  // Sort hanging first by action priority then age desc
  const actionOrder = { book: 0, rebook: 1, monitor: 2, wait: 3 };
  const movementOrder = { hanging: 0, 'in-transit': 1, delivered: 2 };
  results.sort((a, b) => {
...
module.exports = { scanStaleShipments };
