...
function suggestedAction(movement, pickupState, age) {
  ...
  // Book the pickup as soon as we see a hanging label with no booking. The
  // old 'age <= 1 → wait' rule was conservative to let the carrier index
  // fresh labels first, but in practice (a) CP pickups don't depend on
  // label indexing and (b) ShipStation V2 pickups accept fresh label_ids
  // immediately. Waiting a day + another day stranded PO-14547 (Burnaby
  // CP) for 2 days of pickup-less limbo on 2026-04-22 → -24.
  return 'book';
}
