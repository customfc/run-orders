...
async function createGhostPickup({ warehouseCode, carrier, force = false }) {
  ...
  // Resolve ship-from identity from the warehouse's vendor (Prosol / Treeco /
  // Torlys / Biyork / CFC Sechelt) so the ghost label doesn't misbrand
  // non-Prosol warehouses as Prosol.
  const vendorName = loc.vendor || 'Prosol';
  const companyName = vendorName === 'Prosol' ? 'Prosol Inc.' : vendorName;
  const shipFrom = {
    name: `${vendorName} ${loc.city}`,
    company: companyName,
    street1: loc.address,
    city: loc.city,
    state: loc.province,
