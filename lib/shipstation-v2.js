...
function assessCarrierAddress(ship = {}) {
  ...
  // No-comma composite: street + unit + business name in one line, e.g.
  // "5000 Highway 7 East Unit 2006L Auric King Fine Restaurant"
  // (701-7987961-9571407, 2026-09-09: two Purolator 1100238s, a day unshipped,
  // fixed by hand). Flagged only when splitUnitAndBusiness can heal it, so a
  // long-but-plain street never raises a false alarm for a human.
  if (!issues.some((i) => i.code === 'ADDRESS1_COMPOSITE') && splitUnitAndBusiness(ship)) {
    issues.push({
      code: 'ADDRESS1_UNIT_COMPOSITE',
      field: 'street1',
      detail: `street1 carries the unit and a business name in one line — "${street1}"`,
    });
  }
...
/**
 * Split a no-comma composite Address1 of the shape
 *   "<civic number + street> <Unit|Suite|Apt|#…> <unit> [business name…]"
 * into street1 / street2 / company. Order 701-7987961-9571407 (2026-09-09):
 * "5000 Highway 7 East Unit 2006L Auric King Fine Restaurant" — Purolator
 * 1100238 twice, a day unshipped, fixed by hand. Same information-preserving
 * bar as the other self-heals: every token survives verbatim, only the field
 * changes, and all three fields print on the label.
 *
 * Only fires when street1 is over 30 chars (Purolator's line limit) — a short
 * "12 Main St Unit 4" is left exactly as the customer wrote it. Returns null
 * when the shape does not match, when street2 is already occupied, when the
 * street part itself would still exceed 30, or when a business name has nowhere
 * to go (company occupied) — those go to a human rather than a guess.
 */
const UNIT_KEYWORD_RE = /\s+(unit|suite|ste|apt|apartment|room|rm)\.?\s+([A-Za-z0-9-]+)(?:\s+(.*))?$/i;
const UNIT_HASH_RE = /\s+#\s*([A-Za-z0-9-]+)(?:\s+(.*))?$/;

function splitUnitAndBusiness(ship = {}) {
  const raw = String(ship.street1 || '').replace(/\s+/g, ' ').trim();
  if (raw.length <= 30 || raw.includes(',')) return null;
  if (String(ship.street2 || '').trim()) return null;
  let m = raw.match(UNIT_KEYWORD_RE);
  let street2, trailer;
  if (m) { street2 = `${m[1]} ${m[2]}`; trailer = (m[3] || '').trim(); }
  else {
    m = raw.match(UNIT_HASH_RE);
    if (!m) return null;
    street2 = `#${m[1]}`; trailer = (m[2] || '').trim();
  }
  const street1 = raw.slice(0, m.index).trim();
  if (!/^\d+[A-Za-z]?\s+\S+/.test(street1) || street1.length > 30) return null;
  const out = { street1, street2 };
  if (trailer) {
    if (String(ship.company || '').trim()) return null;
    out.company = trailer;
  }
  return out;
}
...
async function ensureValidShipTo(orderId, orderNumber) {
  ...
  // ADDRESS1_UNIT_COMPOSITE: street + unit + business name in one line with no
  // commas (701-7987961-9571407, 2026-09-09). splitUnitAndBusiness re-wraps it
  // into street1 / street2 / company, every token verbatim. Same upsert →
  // verify → re-assess shape as the two heals above.
  if (addrIssues.some((i) => i.code === 'ADDRESS1_UNIT_COMPOSITE')) {
    const split = splitUnitAndBusiness(ship);
    if (split) {
      const upsertUnit = await v1Request('POST', '/orders/createorders', [{ ...order, shipTo: { ...ship, ...split } }]);
      if (upsertUnit.status === 200) {
        await new Promise((r) => setTimeout(r, 800));
        const check = await v1Request('GET', `/orders/${orderId}`);
        if (check.status === 200) {
          const fresh = JSON.parse(check.body);
          if (String(fresh.shipTo?.street1 || '') === split.street1 && String(fresh.shipTo?.street2 || '') === split.street2) {
            console.log(`[ensureValidShipTo] split unit/business out of Address1 for ${orderNumber}: "${ship.street1}" -> "${split.street1}" / "${split.street2}"${split.company ? ` / company "${split.company}"` : ''}`);
            order = fresh;
            ship = order.shipTo || {};
            addrIssues = assessCarrierAddress(ship);
          }
        ...
        console.warn(`[ensureValidShipTo] unit split upsert failed for ${orderNumber}: ${upsertUnit.status} ${upsertUnit.body.slice(0, 120)}`);
      }
    }
  }
