 ...
 *  - "cable_kit":   DITRA-HEAT cable kit boxes, up to max_per_pkg (2) per box
...
function planPackages(items, orderWeight, skuMap) {
  ...
  const rows = (items || []).filter((i) => {
    ...
    // Drop non-physical lines — discount/promo codes carry no SKU and no
    // weight (e.g. Shopify WELCOME10). Previously these fell through to the
    // fallback unit weight (orderWeight/qty) and inflated the default box,
    // over-declaring the label. See project_pipeline_split_ship_bulky_orders.
    const hasSku = i.sku != null && String(i.sku).trim() !== "";
    const w = i.weight && Number(i.weight.value);
    const hasWeight = Number.isFinite(w) && w > 0;
    return hasSku || hasWeight;
  });
  if (!rows.length) {
  ...
  // Solo: each unit its own package by default.
  // Exception: when the SKU sets max_per_pkg, same-SKU multiples bundle
  // together up to that cap (and within max_pkg_weight_lb if set). Used by
  // long thin items like Kerdi Line drain channels — 2× 48" channels strap
  // into one package, not two. Different SKUs still ship separately.
  const solos = units.filter((u) => u.shape === 'solo');
  const solosBySku = new Map();
  for (const u of solos) {
    if (!solosBySku.has(u.sku)) solosBySku.set(u.sku, []);
    solosBySku.get(u.sku).push(u);
  }
  for (const [, group] of solosBySku) {
    const cap = group[0].maxPerPkg || 1;            // default 1 = legacy one-per-package
    const wtCap = group[0].maxPkgWeightLb || Infinity;
    let bin = null;
    for (const u of group) {
      if (!bin || bin.units.length >= cap || (bin.weight + u.unitLb) > wtCap) {
        if (bin) packages.push({
  ...
  // cable_kit: group DITRA-HEAT cable kit boxes together up to max_per_pkg (2).
  // Kept separate from roll_kerdi so cables don't mix with waterproofing rolls
  // in the same physical box.
  const cableKits = units.filter((u) => u.shape === 'cable_kit');
  if (cableKits.length) {
    const cap = cableKits[0].maxPerPkg || 2;
    for (let i = 0; i < cableKits.length; i += cap) {
      const batch = cableKits.slice(i, i + cap);
      packages.push({
        items: aggregateItems(batch),
        totalWeight: { value: Number(batch.reduce((s, u) => s + u.unitLb, 0).toFixed(2)), units: 'pounds' },
        shape: 'cable_kit',
