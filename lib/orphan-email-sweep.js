 ...
 *
 * THREE root causes, all real, all seen in production:
 ...
 *   3. A label is bought by a one-off script that never writes the ledger at
 *      all (2026-08-31, five orders). Causes 1–2 are caught by reading
 *      phases.buy.labels; cause 3 is only caught by reading ShipStation first
 *      and backfilling the ledger — PASS 0 below.
 *   Cause 2 hits every ledger-writing buy path, including `/api/labels/buy`
 *   and telegram /buy, which bypass runPipeline entirely. That is why this
 *   sweep is the backstop rather than something bolted onto one call site.
 *
 * Scans two windows:
...
/**
 * Has a SIBLING label on this order already moved?
 *
 * A void-and-rebuy leaves two labels on one order. The branch ships on the one
 * it was told about — even a voided one — and our newer label never scans. If
 * we then "rescue" that newer label we are asking the branch to ship a SECOND
 * parcel for an order the customer has already received. Mississauga
 * 702-6587132-2979443 delivered 2026-07-28 on voided label 520666210304 while
 * 520667450784 sat unscanned; rescuing it would have shipped a duplicate on a
 * $490 order.
 *
 * Fails CLOSED (returns true, i.e. skip) on a lookup error: a duplicate
 * shipment costs far more than a delayed one.
 */
async function siblingAlreadyShipped(orderNumber, trackingNumber) {
  try {
    const r = await v1Request('GET', `/shipments?orderNumber=${encodeURIComponent(orderNumber)}`);
    if (r.status !== 200) return true;
    const shipments = JSON.parse(r.body).shipments || [];
    const sibs = shipments.filter((s) => s.trackingNumber && s.trackingNumber !== trackingNumber);
    if (!sibs.length) return false;
    for (const sib of sibs) {
      // v1 has no /shipments/{id}/track (404). Tracking events live on the V2
      // label endpoint, keyed se-<shipmentId>, and v2Request returns a raw
      // string body.
      const t = await v2Request('GET', `/v2/labels/se-${sib.shipmentId}/track`).catch(() => null);
      if (!t || t.status !== 200) continue;
      let j = null;
      try { j = typeof t.body === 'string' ? JSON.parse(t.body) : t.body; } catch { continue; }
      const events = (j && j.events) || [];
      // Any physical scan means the goods are already with the carrier.
      // NY = label created, AC = electronically accepted: neither is movement.
      if (events.some((e) => ['IT', 'AT', 'DE', 'EX'].includes(String(e.status_code || '').toUpperCase()))) return true;
    }
    return false;
  } catch { return true; }
}
...
// ── ShipStation-first discovery ─────────────────────────────────────────────
//
// THIRD root cause (2026-08-31): a label bought OUTSIDE runPipeline — a one-off
// script fixing a carrier rejection during the ShipStation subscription-cap
// window — never reaches phases.buy.labels. phaseEmail and this sweep both read
// that same ledger, so the backstop was anchored to the thing it was backing up
// and five orders (1358, 1368, 1370 + two Amazon) were invisible to it at any
// age. The only system that reliably holds every label is ShipStation, so the
// sweep now starts there: enumerate every live shipment in the detect window,
// subtract what the ledger already knows, and write the rest INTO the ledger on
// the day they were bought. From then on they are ordinary labels — same
// void/cancel/sibling checks, same send-or-shadow, same per-day idempotency.

// Standalone-label stubs and hand-emailed CS reships are not orders the branch
// needs a pack-email for. Split-child shipments carry a blank orderNumber and
// are covered by their parent (see reference_shipstation_split_child_blank_order).
const SS_ORDER_SKIP_RE = /^(SEAuto-|RESHIP-)/i;

// ShipStation v1 timestamps are Pacific wall-clock with no offset
// ("2026-08-31T07:59:31.5430000"). Ops-state days are America/Toronto, which is
// always three hours ahead of Pacific (both switch DST on the same dates).
function pacificWallAsUtcMs(s) {
  return Date.parse(String(s || '').replace(/(\.\d+)?$/, '') + 'Z');
}
function pacificWallToUtcMs(s) {
  const wall = pacificWallAsUtcMs(s);
  if (!Number.isFinite(wall)) return NaN;
  const dst = /PDT/.test(new Date(wall).toLocaleString('en-US', { timeZone: 'America/Los_Angeles', timeZoneName: 'short' }));
  return wall + (dst ? 7 : 8) * 3600000;
}
function torontoDayOf(s) {
  const wall = pacificWallAsUtcMs(s);
  return Number.isFinite(wall) ? new Date(wall + 3 * 3600000).toISOString().slice(0, 10) : null;
}

// Mirrors orderSource() in run-orders.js without importing the whole engine.
function sourceFromOrderNumber(orderNumber, s) {
  const n = String(orderNumber || '').trim();
  if (/^\d{3}-\d{7}-\d{7}$/.test(n)) return 'amazon_ca';
  if (/^#?\d{4,6}$/.test(n)) return 'shopify';
  return (s && s.advancedOptions && s.advancedOptions.source) || 'unknown';
}

async function listShipStationLabels(sinceDate) {
  const out = [];
  for (let page = 1; page <= 20; page++) {
    const r = await v1Request('GET', `/shipments?createDateStart=${sinceDate}&pageSize=500&page=${page}&includeShipmentItems=false`);
    if (r.status !== 200) throw new Error(`ShipStation /shipments HTTP ${r.status}`);
...
// Has THIS label physically moved? A manual label with a carrier scan was
// shipped by a branch that was told through a channel we cannot see; emailing
// it again asks for a second parcel. NY/AC are label-only, not movement.
async function labelHasMoved(shipmentId) {
  const t = await v2Request('GET', `/v2/labels/se-${shipmentId}/track`);
  if (!t || t.status !== 200) throw new Error(`track HTTP ${t && t.status}`);
  const j = typeof t.body === 'string' ? JSON.parse(t.body) : t.body;
  return ((j && j.events) || []).some((e) => ['IT', 'AT', 'DE', 'EX'].includes(String(e.status_code || '').toUpperCase()));
}

// Warehouse emails sent by hand from a CS session land in audit.jsonl, not in
// ops-state. Without this, a hand-emailed manual label would be "rescued" into
// a second pack-email to the branch.
function parseManualEmailRecords(text) {
  const out = [];
  for (const line of String(text || '').split('\n')) {
    if (!line || !/email/i.test(line)) continue;
    let e; try { e = JSON.parse(line); } catch { continue; }
    const action = String(e.action || '');
    if (!/email/i.test(action) || /^pipeline-email|^orphan-sweep/.test(action)) continue;
    // `attached` lists the labels the email actually carried. An order that was
    // merely MENTIONED is not covered: the 2026-09-07 "6 parcels" chase named
    // 1370 but attached only 1368, and Sechelt still had no label for 1370.
    // Fall back to the order lists only when the record has no `attached`.
    const nums = (Array.isArray(e.attached) ? e.attached : [].concat(e.orders || [], e.orderNumbers || [], e.orderNumber || []))
      .map((x) => String(x).replace(/^#/, ''));
    for (const n of nums) out.push({ orderNumber: n, at: e.timestamp });
  }
  return out;
}

function readManualEmailRecords() {
  try {
    const auditFile = path.join(__dirname, '..', 'data', 'audit.jsonl');
    if (!fs.existsSync(auditFile)) return [];
    return parseManualEmailRecords(fs.readFileSync(auditFile, 'utf8'));
  } catch { return []; }
}

/**
 * @param live         actually send (default: ORPHAN_SWEEP_LIVE=1)
 * @param listShipments (sinceDate) => [shipment]        — injectable for tests
 * @param manualEmails  () => [{orderNumber, at}]         — injectable for tests
 * @param checkMoved    (shipmentId) => bool (throws on lookup failure) — injectable for tests
...
async function runOrphanSweep({
  live = isLive(),
  listShipments = listShipStationLabels,
  manualEmails = readManualEmailRecords,
  checkMoved = labelHasMoved,
  checkVoided = isLabelVoided,
  checkSibling = siblingAlreadyShipped,
  ...
  const report = {
    ...
    siblingShipped: [],   // order already went out on another label — duplicate risk
    anomalies: [],        // unmapped warehouse — routing config gap
    discovered: [],       // labels bought outside the pipeline, found in ShipStation and added to the ledger
    handEmailed: [],      // subset of discovered that a CS session already emailed by hand (audit.jsonl)
    alreadyMoved: [],     // subset of discovered that already has carrier scans — shipped, never re-emailed
    errors: [],
  };

  const sendDates = new Set(datesBack(SEND_LOOKBACK_DAYS));

  // PASS 0 — ShipStation-first discovery (see the block comment above). Reads
  // every scanned day once; the same state objects feed pass 1 so a backfilled
  // label is classified in this very run. Today is left to the live pipeline.
  const dayStates = {};
  for (const date of report.datesScanned) dayStates[date] = loadDayState(date);
  try {
    const known = new Set(); // tracking numbers the ledger already holds
    for (const st of Object.values(dayStates)) {
      for (const l of Object.values(st.phases?.buy?.labels || {})) {
        if (l.trackingNumber) known.add(String(l.trackingNumber));
        for (const p of (l.packages || [])) if (p && p.trackingNumber) known.add(String(p.trackingNumber));
      }
    }
    const today = opsState.today();
    const since = report.datesScanned[report.datesScanned.length - 1];
    const shipments = await listShipments(since);
    const hand = manualEmails() || [];
    for (const s of shipments) {
      if (!s || s.voided || s.voidDate || !s.trackingNumber || !s.orderId) continue;
      const orderNumber = String(s.orderNumber || '').trim().replace(/^#/, '');
      if (!orderNumber || SS_ORDER_SKIP_RE.test(orderNumber)) continue;
      if (known.has(String(s.trackingNumber))) continue;
      const date = torontoDayOf(s.createDate);
      const atMs = pacificWallToUtcMs(s.createDate);
      if (!date || !Number.isFinite(atMs) || date >= today) continue;
      // Fail closed on an unanswered tracking lookup: leave the label out of
      // the ledger this tick and try again next hour, rather than guess.
      let moved = null;
      try { moved = await checkMoved(s.shipmentId); } catch (e) { moved = null; }
      if (moved === null) {
        report.errors.push({ orderNumber, tracking: s.trackingNumber, error: 'discovery: tracking lookup failed, retry next tick' });
        continue;
      }
      const st = dayStates[date] || (dayStates[date] = loadDayState(date));
      if (!st.phases.buy) st.phases.buy = { labels: {} };
      const at = new Date(atMs).toISOString();
      const labelCost = Number(s.shipmentCost || 0);
      st.phases.buy.labels[String(s.orderId)] = {
        orderNumber,
        source: sourceFromOrderNumber(orderNumber, s),
        shipmentId: s.shipmentId, trackingNumber: s.trackingNumber, labelCost, costWarning: false,
        carrierCode: s.carrierCode || null, serviceCode: s.serviceCode || null, estimatedCost: null,
        warehouseId: s.warehouseId || null,
        packages: [{ shipmentId: s.shipmentId, trackingNumber: s.trackingNumber, labelCost }],
        at,
        origin: 'shipstation-sweep',
        backfilledAt: new Date().toISOString(),
      };
      known.add(String(s.trackingNumber));
      const rec = { date, ageDays: ageDays(date), orderNumber, orderId: s.orderId, tracking: s.trackingNumber, labelCost, warehouseId: s.warehouseId || null };
      const handRec = hand.find((h) => h.orderNumber === orderNumber && Date.parse(h.at) > atMs);
      if (handRec || moved) {
        if (!st.phases.email) st.phases.email = { byWarehouse: {}, byOrder: {}, lastAlertAt: {}, lastLargeEmailAt: {} };
        if (!st.phases.email.byOrder) st.phases.email.byOrder = {};
        st.phases.email.byOrder[String(s.orderId)] = handRec
          ? { warehouse: 'hand-emailed (audit.jsonl)', at: handRec.at }
          : { warehouse: 'already in transit (carrier scan)', at: new Date().toISOString() };
        if (handRec) report.handEmailed.push({ ...rec, emailedAt: handRec.at });
        else report.alreadyMoved.push(rec);
      }
      report.discovered.push(rec);
      saveDayState(st);
      auditLog({ action: 'orphan-sweep-backfill', date, orderId: s.orderId, orderNumber, tracking: s.trackingNumber, warehouseId: s.warehouseId || null, labelCost, moved, handEmailedAt: handRec ? handRec.at : null });
    }
  } catch (e) {
    report.errors.push({ error: `shipstation discovery: ${e.message}` });
  }
  ...
  for (const date of report.datesScanned) {
    const state = dayStates[date] || loadDayState(date);
    const labels = Object.entries(state.phases?.buy?.labels || {});
    if (!labels.length) continue;
    const unEmailed = labels
      .map(([orderId, l]) => ({ orderId: Number(orderId), ...l }))
      // Label-level, not order-level. An order emailed at 18:00 does NOT cover
      // a label bought at 20:20 — the branch ships on the tracking number it was
      // given, our newer label never scans, and the parcel looks stranded for a
      // week. That is exactly what happened to Mississauga 702-6587132-2979443.
      .filter((l) => {
        const emailedAt = opsState.emailedAtForOrder(state, l.orderId);
        if (!emailedAt) return true;                       // never emailed at all
        if (!l.at) return false;                           // no buy time — assume covered
        return Date.parse(l.at) > Date.parse(emailedAt);   // bought after the email
      });
    ...
    for (const l of unEmailed) {
      ...
      try {
        ...
        if (await checkSibling(l.orderNumber, l.trackingNumber)) {
          report.siblingShipped.push({ ...base, reason: 'sibling-label-already-shipped' });
          skipIdsByDate[date].push(l.orderId);
          continue;
        }
...
function signature(report) {
  return [
    ...
    ...report.discovered.map((s) => `D:${s.orderNumber}`),
...
function formatReport(report) {
  ...
  if (report.discovered.length) {
    const hand = new Set(report.handEmailed.map((h) => h.orderNumber));
    const moved = new Set(report.alreadyMoved.map((h) => h.orderNumber));
    const tag = (d) => hand.has(d.orderNumber) ? ', hand-emailed' : moved.has(d.orderNumber) ? ', in transit' : '';
    L.push(`🧾 ${report.discovered.length} label(s) bought outside the pipeline, found in ShipStation and added to the ledger: ${report.discovered.map((d) => `${d.orderNumber} (${d.date}${tag(d)})`).join(', ')}`);
  }
