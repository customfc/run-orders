...
async function runOrphanSweep({
  ...
  for (const date of report.datesScanned) {
    ...
    try {
      const r = await sendEmail({ state, skipOrderIds: skipIds });
      saveDayState(state);
      const count = (r.sent || []).reduce((n, s) => n + (s.orderCount || 0), 0);
      report.sent.push({ date, warehouses: (r.sent || []).map((s) => s.warehouse), count, orderNumbers: sendable.map(({ base }) => base.orderNumber) });
      audit.log({ action: 'orphan-sweep-sent', date, count, warehouses: (r.sent || []).map((s) => s.warehouse), orderNumbers: sendable.map(({ base }) => base.orderNumber) });
    } catch (e) {
...
function signature(report) {
  return [
    ...report.sent.map((s) => `S:${s.date}:${s.count}`),
...
function formatReport(report) {
  ...
  if (report.sent.length) {
    ...
    L.push(`✅ RESCUED ${n} orphan email(s): ${report.sent.map((s) => `${s.date}→${s.warehouses.join('/')}`).join(', ')}`);
  }
