...
async function findPbsiItem(conn, prosolSku) {
  ...
  // Build the variant list we try in order. Salesforce historically stored
  // Schluter Vendor IDs without the slash/hyphen separators that the sku-map
  // (correctly) carries (project_prosol_sku_slashes). Hitting a no-match here
  // used to spawn duplicate auto-created items — checking the stripped form
  // prevents that.
  const variants = [prosolSku];
  const noSlash = prosolSku.replace(/\//g, '');
  if (noSlash !== prosolSku) variants.push(noSlash);
  const noHyphen = prosolSku.replace(/-/g, '');
  if (noHyphen !== prosolSku && !variants.includes(noHyphen)) variants.push(noHyphen);
  const stripped = prosolSku.replace(/[\/-]/g, '');
  if (stripped !== prosolSku && !variants.includes(stripped)) variants.push(stripped);
