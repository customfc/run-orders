  ...
  for (const shipment of needsPO) {
    ...
    try {
      ...
      for (const item of resolvedItems) {
        ...
        // Receive into Amazon Fulfillment immediately. PBSI__Received_Purchase_Order_Line__c
        // is the per-line receipt record; setting PBSI__Type__c='receive' increments the
        // location's quantity-on-hand. Failure here doesn't roll back the PO line — we log
        // and flag so a person can reconcile manually.
        try {
          const receiptId = await sf.create(conn, 'PBSI__Received_Purchase_Order_Line__c', {
            PBSI__Purchase_Order__c: poId,
          ...
          orderResult.receipts.push({ itemId: item.pbsiItemId, qty: item.quantity, receiptId });
        } catch (err) {
          orderResult.errors.push(`Receive at Amazon Fulfillment for ${item.pbsiItemName}: ${err.message}`);
        }
      }

      // Close the PO once every line has been received. Mac directive 2026-05-26:
      // fully-received Amazon POs should end Closed, not Open.
      const allReceived = resolvedItems.length > 0
        && resolvedItems.every((item) => orderResult.receipts.some((r) => r.itemId === item.pbsiItemId));
      if (allReceived) {
        try {
          await conn.sobject('PBSI__PBSI_Purchase_Order__c').update({ Id: poId, PBSI__Status__c: 'Closed' });
          orderResult.closed = true;
        } catch (err) {
          orderResult.errors.push(`Close PO ${orderResult.poNumber}: ${err.message}`);
        }
      }
