...
/**
 * Flatten whatever a failed jsforce/SOQL call throws into something an operator
 * can act on. The 2026-07-24 duplicate-PO incident logged `"error": ""` because
 * the thrown value carried no `.message`, so the audit trail could not say what
 * had broken — only that something had.
 */
function errorText(err) {
  if (!err) return 'unknown error (no error object)';
  const parts = [err.message, err.errorCode, err.name, err.statusCode].filter(Boolean);
...
async function createAmazonPOs({ days = 7, onProgress = () => {} } = {}) {
  ...
  try {
    ...
    // FAIL CLOSED. This is the duplicate guard: without its answer every parcel
    // looks new and gets a second PO. On 2026-07-24 this catch swallowed the
    // failure and continued with an empty set — posSkipped went to 0, 12 parcels
    // that already had POs got another one, and $744.26 of duplicate POs landed
    // in the books as Complete/Received (PO-16108..16119 against PO-16069..16101).
    // The asymmetry is the whole argument: a PO we fail to create is picked up by
    // the next tick, a PO we create twice is money and a vendor argument. So we
    // abort the phase, create nothing, and let phasePos halt + alert.
    results.errors.push({
      step: 'check-existing',
      error: `duplicate guard failed, PO creation aborted (no POs created this run): ${errorText(err)}`,
    });
    return results;
  }
