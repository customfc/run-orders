...
async function createAmazonPOs({ days = 7, onProgress = () => {} } = {}) {
  ...
  for (const shipment of needsPO) {
    ...
    try {
      ...
      if (allReceived) {
        try {
          // mm_Received_Location_Name__c and CFC_Stage__c are Formula (Text)
          // fields now — read-only for everyone, they compute themselves from
          // the receive. Writing them made SF reject this whole update, so
          // every PO stalled at PBSI's auto-flipped 'Complete' instead of
          // 'Received' (found 2026-07-20; broke silently when the fields were
          // converted to formulas). Only writable fields go in the update.
          await conn.sobject('PBSI__PBSI_Purchase_Order__c').update({
            Id: poId,
