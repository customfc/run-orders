...
// First day of the current month, YYYY-MM-DD. The autonomous LIVE sweep must
// NEVER create an SO dated into a prior (likely-closed) accounting month —
// Mac 2026-07-23: "don't fuck with prior accounting months." Backfilling an
// older order is a deliberate, Lynnae-dated operation, not something the cron
// does on its own. Callers can override via minCreateDate (the CLI does, for an
...
async function reconcileShopifySOs({ days = LOOKBACK_DAYS, live = isLive(), minCreateDate = currentMonthStart(), onProgress = () => {} } = {}) {
  const report = { live, days, minCreateDate, scanned: 0, eligible: 0, missing: [], created: [], skipped: [], priorMonth: [], errors: [] };
  ...
  for (const m of report.missing) {
    // Hard floor: never auto-create an SO dated before the current month. An
    // older gap is surfaced (priorMonth) for a deliberate, correctly-dated
    // backfill — the cron never reaches into a closed accounting period.
    if (minCreateDate && m.created_at && m.created_at < minCreateDate) {
      report.priorMonth.push({ order: m.order, created_at: m.created_at });
      onProgress({ type: 'status', message: `[so-reconcile] ⏸ ${m.order} (${m.created_at}) is before ${minCreateDate} — prior month, NOT auto-created (manual backfill needed)` });
      continue;
    }
    try {
...
function formatReport(r) {
  ...
  if ((r.priorMonth || []).length) L.push(`⏸ prior-month (NOT auto-created, need Lynnae-dated backfill): ${r.priorMonth.map(m => `${m.order} (${m.created_at})`).join(', ')}`);
