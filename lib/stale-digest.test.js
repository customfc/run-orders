...
test('a NEW stuck-in-transit speaks even with nothing new hanging', () => {
...
test('a KNOWN stuck parcel does not re-fire the next day', () => {
  // firing stuck unconditionally would have left the daily-noise bug alive in
  // this bucket — 4 stuck parcels would mean a message every day forever
  const scan = scanOf({ trackingNumber: 'C', orderNumber: 'C', movement: 'stuck-in-transit', age: 11, warehouseName: 'X', carrier: 'p' });
  const first = buildDigest({ scan, state: empty, now: NOW });
  const second = buildDigest({
    scan: scanOf({ trackingNumber: 'C', orderNumber: 'C', movement: 'stuck-in-transit', age: 12, warehouseName: 'X', carrier: 'p' }),
    state: first.state, now: new Date('2026-07-28T12:00:00Z'),
  });
  assert.equal(second.shouldSend, false, 'same stuck parcel, one day older — nothing new to say');
});
