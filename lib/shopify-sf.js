...
async function createShopifySoPo({ shopifyOrder, onProgress = () => {} } = {}) {
  ...
  try {
    ...
    results.poId = await sf.create(conn, 'PBSI__PBSI_Purchase_Order__c', {
      PBSI__Account__c: PROSOL_VENDOR_ID,
      PBSI__Order_Date__c: new Date().toISOString().slice(0, 10),
      PBSI__Status__c: 'Open',
      PBSI__Shipping_Instructions__c: `Shopify ${shopifyOrder.orderNumber} — ${customerName} — ${itemDesc}`.slice(0, 255),
    });
