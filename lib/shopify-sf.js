...
// Skip-if-exists guard: prevents duplicate SOs when the Sechelt warehouse
// user or Mac has already hand-entered an SO for this Shopify order.
// Called before the SO create step. Two layers of detection:
//   1. Strong — a PO with matching tracking_code exists (we've created this
//      one already in a prior run).
//   2. Weaker — any SO under the Shopify account within ±2 days of the
//      Shopify order date. False positives here = we skip a new order
//      because someone shipped a different Shopify order on the same day;
//      the missed SO can be created manually by Mac / Sechelt. Better
//      than false negatives (double SOs confuse month-end close).
async function findExistingShopifySo(conn, shopifyOrder, trackingNumber) {
  if (trackingNumber) {
    ...
    if (pos.length) return { reason: 'PO already exists for this tracking number', poName: pos[0].Name, soId: pos[0].PBSI__Sales_Order__c, strong: true };
  }
  const orderDate = (shopifyOrder.createdAt || '').slice(0, 10);
  if (orderDate) {
    ...
    const sos = await sf.query(conn, `
      SELECT Id, Name, PBSI__Order_Date__c, CreatedDate, CreatedBy.Name
      FROM PBSI__PBSI_Sales_Order__c
      WHERE PBSI__Customer__c = '${SHOPIFY_ACCOUNT_ID}'
        AND PBSI__Order_Date__c >= ${fmt(start)}
        AND PBSI__Order_Date__c <= ${fmt(end)}
      ORDER BY CreatedDate DESC LIMIT 10
    `);
    if (sos.length) {
      const existing = sos.map((s) => `${s.Name} (${s.PBSI__Order_Date__c}, by ${s.CreatedBy?.Name || '?'})`);
      return { reason: `${sos.length} SO(s) under Shopify account within ±2d of Shopify order date ${orderDate}`, candidates: existing, strong: false };
    }
  }
  return null;
}

async function createShopifySoPo({ shopifyOrder, onProgress = () => {}, trackingNumber = null, carrierCode = null } = {}) {
