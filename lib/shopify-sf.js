  ...
  for (const item of shopifyOrder.items) {
    ...
    try {
      ...
      if (!sfItem) {
        ...
        const translated = mapped && typeof mapped === 'object' ? mapped.api_sku : null;
        if (translated && translated !== item.sku && !SKIP_API_SKUS.has(translated)) {
          sfItem = await findItemBySku(conn, translated);
          lookedUp.push(translated);
        }
      }
