...
async function findItemBySku(conn, sku) {
  ...
  // Try slash/hyphen variants of the Prosol SKU because SF historically
  // stored Schluter Vendor IDs without separators (KERDIFIXBW, DITRADRAIN25M)
  // while sku-map carries the slashed form (KERDIFIX/BW, DITRA-DRAIN25M) per
  // project_prosol_sku_slashes. Without this fallback we spawn duplicate
  // auto-created PBSI items every time an order hits a legacy SF row.
  const variants = [skuStr];
  const noSlash = skuStr.replace(/\//g, '');
  if (noSlash !== skuStr) variants.push(noSlash);
  const noHyphen = skuStr.replace(/-/g, '');
  if (noHyphen !== skuStr && !variants.includes(noHyphen)) variants.push(noHyphen);
  const stripped = skuStr.replace(/[\/-]/g, '');
  if (stripped !== skuStr && !variants.includes(stripped)) variants.push(stripped);

  for (const v of variants) {
    const escaped = v.replace(/'/g, "\\'");
    const records = await sf.query(conn, `SELECT Id, Name, PBSI__salesprice__c, PBSI__Vendor_Item_ID__c, PBSI__Default_Vendor_Name__c FROM PBSI__PBSI_Item__c WHERE PBSI__Vendor_Item_ID__c = '${escaped}' LIMIT 1`);
...
async function createPbsiItem(conn, { mfgSku, prosolSku, productName, title, entry }) {
  ...
  const retail = resolvedEntry && resolvedEntry.retail_cad ? Number(resolvedEntry.retail_cad) : null;
  const cost = lookupCanonicalCost(prosolSku, mfgSku);
  ...
  return {
    ...
    PBSI__Cost__c: cost,
    autoCreated: true,
    costSource: cost !== null ? 'canonical' : null,
  };
}
...
module.exports = { fetchShopifyOrder, createShopifySoPo, createPbsiItem };
