...
function planKeyFor(draftId, vendor, bucket) {
  return `${draftId}-${vendor}${bucket ? '-' + bucket : ''}-inbound`;
}
...
async function runAll(opts) {
  const { draftId, vendor, bucket, source, emailTo, emailCc, transportMode, pageType, skipEmail, onProgress } = opts;
  if (!draftId) throw new Error('draftId required');
  if (!vendor) throw new Error('vendor required');
  const progress = onProgress || (() => {});

  const planKey = planKeyFor(draftId, vendor, bucket);
  ...
  if (!existing || !existing.inboundPlanId) {
    ...
    if (bucket) args1.push(`--bucket=${bucket}`);
...
/**
 * Bucket-aware wrapper around runAll. Fires the SP-API inbound plan
 * orchestration for a specific (vendor, bucket) slice of a draft. Requires
 * confirm:true to actually walk — absent that, returns a dry-run plan of
 * what WOULD be created without any Amazon call.
 *
 * cartonDims are persisted onto the plan state (inbound-plan JSON) so
 * subsequent steps + the labels email can surface them. Amazon's v2024-03-20
 * packing step doesn't currently accept caller-supplied carton dims in the
 * scripted path — it returns packing options Amazon inferred from the item
 * catalog. Storing cartonDims here is forward-compatible for when we wire
 * custom packing groups, and documents what the vendor told us regardless.
 */
async function runForBucket(opts) {
  const { draftId, vendor, bucket, cartonDims, confirm, onProgress } = opts;
  if (!draftId) throw new Error('draftId required');
  if (!vendor) throw new Error('vendor required');
  ...
  // Dry-run: no SP-API calls. Returns what the live flow would do.
  if (!confirm) {
    return {
      ok: true,
      dryRun: true,
      ...
      wouldRun: [
        '1. create-inbound',
        '2. packing',
        '3. placement (IRREVERSIBLE — locks Amazon FC)',
        '4. transport (APC UPS)',
        '5. labels (FNSKU + download)',
      ],
      cartonDims: cartonDims || null,
      note: 'Pass confirm:true to actually fire SP-API calls.',
    };
  }

  // Stamp cartonDims onto the plan state before step 1 runs. runAll loads/
  // re-saves plan state between steps, so we persist via the plans module.
  if (cartonDims) {
    let state = plans.load(planKey);
    if (!state) {
      // Pre-create a stub so cartonDims survives the first script invocation
      // (plans.create is called inside step 1 and will overwrite — so we read
      // it back after and re-stamp).
      // Simpler: let step 1 run, then re-stamp after.
    } else {
      state.cartonDims = cartonDims;
      plans.save(state);
    }
  }

  const result = await runAll({ ...opts, planKey });

  // Re-stamp cartonDims onto the final state in case step 1 created a fresh
  // plan state that didn't include our dims.
  if (cartonDims) {
  ...
  return { ...result, cartonDims: cartonDims || null };
}

module.exports = { runAll, runForBucket, planKeyFor };
