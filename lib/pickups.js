...
async function scanShippedLabels({ days = 14 } = {}) {
  ...
  // Give split children their parent back BEFORE the blank-orderNumber filter
  // below drops them. Box 2 of a multi-package order comes back from the API
  // with no orderNumber and a phantom warehouseId, so it was invisible here and
  // never offered for collection — Elvin Kao's three Downsview rolls sat four
  // days for exactly this reason, and a Canada Post pickup could not even be
  // booked because the phantom origin has no branch code.
  const repaired = splitChildren.repair(shipments, { knownWarehouseIds: new Set(Object.keys(WAREHOUSE_BY_SS_ID)) });
  if (repaired.orderFixed || repaired.warehouseFixed) {
    console.log(`  🔗 split children: ${repaired.orderFixed} order number(s), ${repaired.warehouseFixed} warehouse(s) restored`);
  }
  if (repaired.unresolved) {
    console.log(`  ⚠️  ${repaired.unresolved} shipment(s) with no order number and no buy record — still skipped`);
  }
