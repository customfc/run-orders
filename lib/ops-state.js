...
// When the branch email for this order went out, or null if it never did.
// The email record does not store WHICH label it carried, so callers compare
// this against the label's purchase time: a label bought after the email was
// sent was demonstrably not in it.
function emailedAtForOrder(state, orderId) {
  const rec = (state.phases.email.byOrder || {})[String(orderId)];
...
module.exports = {
  ...
  emailAlreadySent, orderAlreadyEmailed, emailedAtForOrder, recordEmailSent,
