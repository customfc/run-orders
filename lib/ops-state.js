...
function recordLabelBought(state, { orderId, orderNumber, shipmentId, trackingNumber, labelCost, costWarning, carrierCode, serviceCode, estimatedCost, warehouseId, packages, source }) {
...
function recordPo(state, { trackingNumber, poNumber, poId, soNumber, soId }) {
  state.phases.pos.byTracking[trackingNumber] = {
    poNumber,
    poId,
...
function recordEmailSent(state, { warehouseKey, orderCount, orderIds }) {
  ...
  state.phases.email.byWarehouse[warehouseKey] = { orderCount, at };
  // Per-order idempotency source of truth
  if (!state.phases.email.byOrder) state.phases.email.byOrder = {};
  for (const id of (orderIds || [])) {
    state.phases.email.byOrder[String(id)] = { warehouse: warehouseKey, at };
  }
  save(state);
}
...
function recordAlert(state, orderId) {
  if (!state.phases.email.lastAlertAt) state.phases.email.lastAlertAt = {};
  state.phases.email.lastAlertAt[String(orderId)] = new Date().toISOString();
  save(state);
}
