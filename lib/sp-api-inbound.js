...
async function listShipmentBoxes(inboundPlanId, shipmentId, { pageSize = 20 } = {}) {
  return _request('GET', `${BASE}/inboundPlans/${inboundPlanId}/shipments/${shipmentId}/boxes?pageSize=${pageSize}`);
}

// FNSKU per-unit item labels. The ONLY label endpoint in v2024-03-20 that
// returns a signed PDF URL. Synchronous (no operationId).
// There is no box-label or BOL endpoint in v2024-03-20 — box-level
// identification relies on the shipment's shipmentConfirmationId (FBA…)
// returned by getShipment after transport confirmation.
//
// pageType options (STANDARD_FORMAT): 'Letter_30' (US Avery 5160, 30/sheet),
// 'A4_21', 'A4_24', 'A4_27', 'A4_40_52x29', 'A4_44_48x25', etc.
async function createMarketplaceItemLabels({
  mskuQuantities,
  marketplaceId,
  ...
  if (!Array.isArray(mskuQuantities) || !mskuQuantities.length) {
    throw new Error('mskuQuantities required');
  }
  const mid = marketplaceId || process.env.AMAZON_SP_MARKETPLACE_ID?.replace(/"/g, '');
  if (!mid) throw new Error('marketplaceId required');
  const body = {
    marketplaceId: mid,
    labelType,
    localeCode,
    mskuQuantities: mskuQuantities.map((m) => ({ msku: m.msku, quantity: Number(m.quantity) })),
    ...(labelType === 'STANDARD_FORMAT' ? { pageType } : {}),
    ...(labelType === 'THERMAL_PRINTING' && height ? { height } : {}),
    ...(labelType === 'THERMAL_PRINTING' && width ? { width } : {}),
  };
  return _request('POST', `${BASE}/items/labels`, body);
}
...
// listDeliveryWindowOptions returns EMPTY until generateDeliveryWindowOptions
// has run and its async operation reaches SUCCESS.
async function generateDeliveryWindowOptions(inboundPlanId, shipmentId) {
  return _request('POST', `${BASE}/inboundPlans/${inboundPlanId}/shipments/${shipmentId}/deliveryWindowOptions`, {});
}
