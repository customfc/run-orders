...
async function sendWarehouseEmail({ warehouse, orders, attachments, to, cc, subject, greeting }) {
  const transport = createTransport();
  const toAddr = to || TO_EMAIL;
  const ccAddr = cc === null ? undefined : (cc || CC_EMAIL);
  const subj = subject || `Order - ${warehouse}`;
  ...
  for (const order of orders) {
    ...
    bodyHtml += `<strong>Order:</strong> ${escHtml(order.orderNumber)}<br>`;
...
module.exports = { sendWarehouseEmail, sendEmail };
