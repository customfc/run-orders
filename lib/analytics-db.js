...
function migrate(db) {
  ...
  for (const { table, col, type } of additiveColumns) {
    try {
      ...
      // "duplicate column" is the normal path once the column exists. Any other
      // error used to be swallowed silently, which hid a real failure that broke
      // ETL + cancel-poll for a whole day. Log it loudly instead.
      if (!/duplicate column/i.test(e.message)) {
        console.error(`[analytics-db] ALTER TABLE ${table} ADD ${col} ${type} failed:`, e.message);
      }
    }
  }
