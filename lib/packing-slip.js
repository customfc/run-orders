...
function buildHtml({ poNumber, soNumber, date, vendor, orderNumber, tracking, carrier, shipTo, items, packages, notes }) {
  // City line only renders when there's an actual locality — a will-call /
  // carrier-collect slip (no customer address disclosed to Prosol) carries the
  // instruction in name/street1 and leaves city/state/postal blank, so we skip
  // the line entirely rather than print a stray ", ".
  const cityLine = [shipTo && shipTo.city, shipTo && shipTo.state, shipTo && shipTo.postalCode].some(Boolean)
    ? `${esc(shipTo.city || '')}, ${esc(shipTo.state || '')} ${esc(shipTo.postalCode || '')}`
    : '';
...
<body>
  ...
  ${notes ? `
  <div style="margin-top:8px;padding:10px 12px;border:2px solid #b00;background:#fff5f5;border-radius:4px">
    <div style="font-weight:700;color:#b00;margin-bottom:4px;text-transform:uppercase;font-size:11px;letter-spacing:0.5px">Special Instructions</div>
    <div style="font-size:12px;color:#222;white-space:pre-wrap">${esc(notes)}</div>
  </div>
  ` : ''}
