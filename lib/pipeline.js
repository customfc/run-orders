...
const amazonFulfillment = require('./amazon-fulfillment');
...
const { itemDisplayName } = require('./sample-item');
...
async function buyOneLabel({ assignment }) {
  ...
  // Step 1b: for an Amazon order, try to confirm each box to Amazon separately
  // so the buyer sees every tracking number instead of only the first. This is
  // the defect that cost us 701-4387228-0916238: three rolls delivered
  // 2026-08-03, Amazon showed one tracking, the estimate lapsed, and the buyer
  // opened a return on $2,190 of membrane he already had.
  //
  // It must happen BEFORE markasshipped. markasshipped confirms the whole
  // order, after which every unit reads as fulfilled and there is no remainder
  // left to attach a second tracking number to — verified on that order:
  // one line item, qty_ordered 3, qty_shipped 3.
  //
  // Shadow unless AMAZON_MULTI_TRACKING_LIVE=1, and any failure falls through
  // to markasshipped below. An unconfirmed order is a late-shipment defect
  // against account health, which is strictly worse than a missing tracking
  // number, so confirmation must never depend on this succeeding.
  let multiTracking = null;
  if (assignment.source === 'amazon_ca' || /^\d{3}-\d{7}-\d{7}$/.test(String(assignment.orderNumber || ''))) {
    try {
      const items = await sp.getOrderItems(assignment.orderNumber);
      multiTracking = await amazonFulfillment.confirmPackages({
        amazonOrderId: assignment.orderNumber,
        packages: bought,
        orderItems: (items && (items.payload?.OrderItems || items.OrderItems)) || [],
        carrierCode: assignment.carrierCode,
        shipDate,
      });
      if (multiTracking.submitted) {
        console.log(`[buyOneLabel] ${assignment.orderNumber}: pushed ${bought.length} tracking numbers to Amazon (feed ${multiTracking.feedId})`);
      } else {
        console.log(`[buyOneLabel] ${assignment.orderNumber}: multi-tracking not sent — ${multiTracking.reason}`);
        if (multiTracking.xml) {
          console.log(`[buyOneLabel] shadow feed for ${assignment.orderNumber}:\n${multiTracking.xml}`);
          // Ping while it's still shadow. These land ~2x/week, so the signal is
          // rare enough to be worth a message and too rare to notice in a log.
          // Reviewing one real feed is the gate before AMAZON_MULTI_TRACKING_LIVE=1.
          try {
            await telegram.notify(
              'debug',
              `📦 Multi-tracking SHADOW — ${assignment.orderNumber}`,
              `${bought.length} boxes, Amazon would have received all ${bought.length} tracking numbers instead of 1.\n\n`
              + bought.map((b, i) => `  ${i + 1}. ${b.trackingNumber}`).join('\n')
              + `\n\nFull feed XML is in logs/server.log. Arm with AMAZON_MULTI_TRACKING_LIVE=1 + restart.`,
            );
          } catch { /* notification must never affect the buy */ }
        }
      }
    } catch (e) {
      console.warn(`[buyOneLabel] multi-tracking skipped for ${assignment.orderNumber}: ${e.message}`);
    }
  }

  // Step 2: mark the parent order shipped so it leaves Awaiting Shipment
  // and the primary tracking pushes to the sales channel. Skipped only when
  // the per-package feed actually went through, since that already confirmed
  // every unit — re-confirming would double-ship the order on Amazon's side.
  // Do NOT void the labels on markasshipped failure — they're valid, packages
  // will ship physically. Just surface the failure loud and let ops recover.
  if (multiTracking && multiTracking.submitted) {
    return {
      ...
      packages: bought,
      amazonFeedId: multiTracking.feedId,
    };
  }
...
async function runPipeline({ phases = PHASES, dryRun = false, onProgress = () => {}, source = 'manual' } = {}) {
  ...
  const orderLines = labels.map((l) => {
    ...
    const itemStr = items.length
      ? items.map((i) => `${itemDisplayName(i)}${i.quantity > 1 ? ` ×${i.quantity}` : ''}`).join(', ')
      : l.orderNumber || '?';
