  ...
  if (shopifyLabels.length) {
    ...
    for (const lbl of shopifyLabels) {
      ...
      try {
        ...
        if (r && r.poNumber && trk) {
          opsState.recordPo(state, { trackingNumber: trk, poNumber: r.poNumber, poId: r.poId });
        }
        shopifyOrders.push({
  ...
  for (const { b, order } of orderRecords) {
    ...
    whMap[whName].orders.push({
      ...
      items: items.flatMap((i) => {
        ...
        // Fallback: still use sku-map product name + prosol_sku if entry exists,
        // instead of showing the raw Amazon ASIN and verbose Amazon title.
  ...
  for (let i = 0; i < entries.length; i++) {
    ...
    try {
      ...
      for (const o of whOrders) {
        ...
        const labelAttachResults = { ok: 0, failed: [] };
        for (let p = 0; p < pkgShipmentIds.length; p++) {
          ...
          try {
            ...
            if (labelPdf) {
              ...
              labelAttachResults.ok++;
            } else {
              labelAttachResults.failed.push({ shipmentId: sid, reason: 'downloadLabelPdf returned null' });
            }
          } catch (e) {
            labelAttachResults.failed.push({ shipmentId: sid, reason: e.message });
          }
        }
        if (labelAttachResults.failed.length) {
          // Don't swallow — surface missing labels so Mac/ops see when warehouse
          // emails go out with fewer labels than packages (the 702-7750339-2624250
          // failure mode from 2026-04-21).
          const msg = `Warehouse email for ${o.orderNumber}: ${labelAttachResults.ok}/${pkgShipmentIds.length} labels attached. Failed: ${labelAttachResults.failed.map((f) => `${f.shipmentId}(${f.reason})`).join(', ')}`;
          console.error(`[phaseEmail] ${msg}`);
          audit.log({ action: 'pipeline-email-label-download-partial', orderNumber: o.orderNumber, ok: labelAttachResults.ok, expected: pkgShipmentIds.length, failed: labelAttachResults.failed });
          progress(onProgress, { phase: 'email', type: 'warning', orderNumber: o.orderNumber, message: msg });
        }
        ...
        try {
          ...
          if (slipPdf) {
            ...
            attachments.push({ filename: `PackingSlip-${slipId}.pdf`, content: slipPdf });
          }
        } catch (e) {
          console.error(`[phaseEmail] packing-slip generation failed for ${o.orderNumber}:`, e.message);
          audit.log({ action: 'pipeline-email-packing-slip-failed', orderNumber: o.orderNumber, error: e.message });
          progress(onProgress, { phase: 'email', type: 'warning', orderNumber: o.orderNumber, message: `packing slip generation failed: ${e.message}` });
        }
      }
      ...
      opsState.recordEmailSent(state, {
        ...
        orderCount: whOrders.length,
        orderIds: whOrders.map((o) => o.orderId).filter((id) => id != null),
      });
      results.sent.push({ warehouse, orderCount: whOrders.length });
      audit.log({ action: 'pipeline-email-prosol', warehouse, orderCount: whOrders.length, orderIds: whOrders.map((o) => o.orderId).filter((id) => id != null), success: true });
