...
async function phaseEmail({ state, onProgress }) {
  ...
  for (const { b, order } of orderRecords) {
    ...
    const po = posOrders.find((p) => p.trackingNumber === b.trackingNumber);
    (whMap[whName] = whMap[whName] || []).push({
      ...
      shipTo: order.shipTo,
      items: (order.items || []).map((i) => ({ sku: i.sku, name: i.name, quantity: i.quantity, unitPrice: i.unitPrice })),
    });
  }
...
async function runPipeline({ phases = PHASES, dryRun = false, onProgress = () => {}, source = 'manual' } = {}) {
  ...
  try {
    ...
    if (phases.includes('buy')) {
      ...
      bought = [...buy.bought, ...buy.skipped.filter((s) => s.shipmentId)];
    }
    ...
    if (phases.includes('email')) {
      const email = await phaseEmail({ state, boughtLabels: bought, posOrders, onProgress });
