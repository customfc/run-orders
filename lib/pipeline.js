...
const { v1Request, fetchShippedShipments, downloadLabelPdf, normalizeOrderAddress } = require('./shipstation-v2');
...
async function buyOneLabel({ assignment }) {
  ...
  if (packages.length === 1) {
    // Normalize province code before buying label
    if (assignment.shipTo?.state) {
      await normalizeOrderAddress(assignment.orderId, assignment.shipTo);
    }
  ...
  // Normalize province code before buying labels
  if (order.shipTo?.state) {
    await normalizeOrderAddress(assignment.orderId, order.shipTo);
  }
