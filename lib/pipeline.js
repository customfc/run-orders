...
const { v1Request, fetchShippedShipments, downloadLabelPdf, ensureValidShipTo, toInchDimensions } = require('./shipstation-v2');
...
async function buyOneLabel({ assignment }) {
  ...
  if (packages.length === 1) {
    ...
    const shipToInfo = await ensureValidShipTo(assignment.orderId, assignment.orderNumber);
    ...
    // Pass explicit inch dimensions so ShipStation can't rate the order's stored
    // dims in the wrong unit. Amazon.ca stores dims in CM; createlabelfororder
    // otherwise rates them as INCHES (a 64×30×10cm niche board → 64×30×10in →
    // ~115 lb dim weight → $66 instead of ~$15). See toInchDimensions.
    const inchDims = toInchDimensions(shipToInfo && shipToInfo.dimensions);
    if (inchDims) {
      payload.dimensions = inchDims;
      console.log(`[buy] ${assignment.orderNumber}: dims ${JSON.stringify(shipToInfo.dimensions)} → ${inchDims.length}×${inchDims.width}×${inchDims.height} in (label rating)`);
    }
...
async function phaseEmail({ state, onProgress, skipOrderIds = [] }) {
